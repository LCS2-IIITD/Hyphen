...
test_path = '../../../Data/test//test_data_process/mr_test.csv'
...
class MyModel(nn.Module):
    ...
    def forward(self, input_ids, attn_masks, token_type_ids):
        ...
        encoder_out = torch.cat(tuple([encoder_out.hidden_states[i] for i in [-1, -2, -3, -4]]),
                                dim=-1)  # [bs, seq_len, hidden_dim*4]--32 128 3072
        # encoder_out  #([16, 128, 3072])

        out = encoder_out.unsqueeze(1)  # ([16, 1, 128, 3072])
...
def test(model, dataloader, with_labels=False):
    ...
    # 将预测结果 写回pred文件中
    pd.DataFrame(data=pred_label, index=range(len(pred_label))).to_csv(predict_class_id_path, header=['task1'],
                                                                       encoding='utf-8')
    ...
    with open(predict_class_id_path, encoding='utf-8') as f:
        ...
        ids_test = df_test['text_id'].tolist()
...
if __name__ == '__main__':
    ...
    ## 训练及验证
    train_eval(model, criterion, optimizer, train_loader, val_loader, epochs=20)  # 训练轮数
    ...
    # set_seed(10)  # Set all seeds to make results reproducible
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    #
