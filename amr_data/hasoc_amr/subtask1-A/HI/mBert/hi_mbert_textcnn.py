...
output_model = './models/mbert+textcnn_models.pth'
best_score = 0  # 初始化 最好的得分为0
batch_size = 32

train_path = '../../../Data/task1_data/subtask1-A/sub-A_process/hi/train.csv'
dev_path = '../../../Data/task1_data/subtask1-A/sub-A_process/hi/dev.csv'
# test_path='/content/drive/MyDrive/hasoc2021/Data/test/hi_Hasoc2021_test_task1.csv'
test_path =  '../../../Data/test/test_data_process/hi_test.csv'

predict_class_id_path = 'result/mbert+textcnn/predict_class_id.csv'
result_path = 'result/mbert+textcnn/classify_result.csv'
...
class MyModel(nn.Module):
    ...
    def __init__(self, freeze_bert=False, model_name=model_name, hidden_size=768, num_classes=2):
        ...
        self.convs = nn.ModuleList(
            [nn.Conv2d(1, num_filters, (k, hidden_size * 4)) for k in filter_sizes])
        ...
        self.fc = nn.Sequential(
            nn.Dropout(p=0.4),
            # 256*3 ---> 2
            nn.Linear(num_filters * len(filter_sizes), num_classes, bias=False),
            # nn.Softmax(),
        )

    def conv_and_pool(self, x, conv):
        x = F.relu(conv(x)).squeeze(3)
        # print(x.shape) #([16, 256, 94])
        x = F.max_pool1d(x, x.size(2)).squeeze(2)
        # print(x.shape #([16, 256])
        return x

    # 前向传播
    def forward(self, input_ids, attn_masks, token_type_ids):
        encoder_out = self.bert(input_ids, token_type_ids=token_type_ids, attention_mask=attn_masks)
        # print(len(outputs))

        encoder_out = torch.cat(tuple([encoder_out.hidden_states[i] for i in [-1, -2, -3, -4]]),
                                dim=-1)  # [bs, seq_len, hidden_dim*4]--32 96 3072
        # encoder_out  #([16, 96, 3072])

        out = encoder_out.unsqueeze(1)  # ([16, 1, 96, 3072])

        out = torch.cat([self.conv_and_pool(out, conv) for conv in self.convs], 1)
        # print(out.shape)  #([16, 768])

        out = self.dropout(out)
        logits = self.fc(out)  # bs class_num 16 2
        # print(logits.shape)  #bs num_classes - 16 2
        return logits
...
def test(model, dataloader, with_labels=False):
    ...
    # 将预测结果 写回pred文件中
    pd.DataFrame(data=pred_label, index=range(len(pred_label))).to_csv(predict_class_id_path,header=['task1'], encoding='utf-8')
    ...
    with open(predict_class_id_path, encoding='utf-8') as f:
        ...
        # ids_test = df_test['tweet_id'].tolist()
        ids_test = df_test['text_id'].tolist()  #清洗后的测试数据

        # 将预测结果 写回t文件中
        pand.DataFrame({'id': ids_test, 'label': predic_col}).to_csv(result_path, index=False)
