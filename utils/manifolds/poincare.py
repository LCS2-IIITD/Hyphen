...
class PoincareBall(Manifold):
    ...
    def sqdist(self, p1, p2, c):
        sqrt_c = c ** 0.5
        test = sqrt_c * self.mobius_add(self.proj(-p1, c), self.proj(p2, c), c, dim=-1).norm(dim=-1, p=2, keepdim=False)
        assert torch.max(test) <= 1
        assert torch.min(test) >= -1
        dist_c = artanh(sqrt_c * self.mobius_add(-p1, p2, c, dim=-1).norm(dim=-1, p=2, keepdim=False))
    ...
    def logmap0(self, p, c, monitor = False):
        sqrt_c = c ** 0.5
    ...
    def mobius_matvec(self, m, x, c, monitor = False):
        x = self.proj(x, c)
        u = self.logmap0(x, c)
        mu = u @ m.transpose(-1, -2)
        if monitor: print(x, u, mu, self.expmap0(mu, c))
